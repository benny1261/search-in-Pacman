  ...
  """
    ...
    """
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                list_visited_corner = list(visited_corner)
                if next_node in self.corners:
                    if next_node not in list_visited_corner:
                        list_visited_corner.append(next_node)
                successor = ((next_node, list_visited_corner), action, 1)
...
def minmanhattan(unserched_corner, cur_pos):
    ...
    for loc in unserched_corner:
        dis = abs(loc[0] - cur_pos[0]) + abs(loc[1] - cur_pos[1]) + minmanhattan([temp for temp in unserched_corner if temp != loc], loc)
        possibilities.append(dis)
